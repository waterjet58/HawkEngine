...
namespace Hawk {
	...
	Model::Model(VulkanContext& context, std::vector<Vertex>& vertices) : _context(context)
	{
	...
	void Model::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
		...
		void* data;
		vkMapMemory(_context.getDevice(), _vertexBufferMemory, 0, bufferSize, 0, &data);

		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(_context.getDevice(), _vertexBufferMemory);
